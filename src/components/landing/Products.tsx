...
export function Products() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchProducts({ data: { featured: true, limit: 6 } }),
    staleTime: 60_000,
  });
      ...
      <div className="max-w-[1600px] mx-auto px-5 lg:px-10">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-end mb-12 lg:mb-20">
          <div>
            ...
            <h2 className="font-display italic text-5xl lg:text-8xl leading-[0.9]">Trending<br />Now<span className="text-brass">.</span></h2>
          </div>
          <p className="max-w-md text-coal/70 leading-relaxed lg:text-lg lg:justify-self-end">
            Hand-picked this week — selected for craft, value, and the way they fit into everyday life across Pakistan.
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-16 lg:gap-x-10 lg:gap-y-24">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="aspect-[3/4] bg-bone/50 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-16 lg:gap-x-10 lg:gap-y-24">
            {featured.map((p, i) => (
              <article key={p.id} className="group">
                <Link to="/products/$productId" params={{ productId: p.slug }} className="block" data-cursor="view">
                  <div className="relative aspect-[3/4] bg-bone overflow-hidden mb-4 lg:mb-5 img-bone-grade">
                    <img
                    ...
                    />
                    {p.badge && <span className="absolute top-3 left-3 bg-brass text-coal eyebrow px-2 py-1">{p.badge}</span>}
                    <span
                      role="button"
                      aria-label="Wishlist"
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <Link to="/products/$productId" params={{ productId: p.slug }}
                      className="block font-display italic text-xl lg:text-2xl leading-tight hover:text-brass transition line-clamp-2">
                      {p.name}
                    </Link>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-medium">Rs {Number(p.price).toLocaleString()}</p>
                    {p.old_price && <p className="text-[10px] text-coal/40 line-through mt-0.5">Rs {Number(p.old_price).toLocaleString()}</p>}
                  </div>
                </div>
              </article>
            ))}
